              ...
              {paginatedUpdates.length > 0 ? (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
                    {paginatedUpdates.map((item) => (
                      <div
                        key={item.id}
                        className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs hover:shadow-md hover:border-primary-blue/40 transition-all flex flex-col justify-between group"
                      >
                        <div>
                          <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider flex items-center gap-1.5 ${item.badgeBg} ${item.badgeColor}`}>
                                {item.icon}
                          ...
                          <Link href={item.href} className="block group mb-2">
                            <h3 className="text-sm sm:text-base font-bold text-slate-900 group-hover:text-primary-blue transition-colors leading-snug line-clamp-2 min-h-[40px]">
                              {item.title}
                            </h3>
                          </Link>
                        ...
                        <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 text-xs mt-auto">
                          <span className="text-slate-500 text-[11px] font-medium truncate max-w-[140px] sm:max-w-[170px]" title={item.meta}>
                            {item.meta}
                          </span>
                  ...
                  {/* PAGINATION CONTROLS */}
                  {totalPages > 1 && (
                    <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
                      <p className="text-xs text-slate-500 font-medium">
                        Halaman <span className="font-bold text-primary-navy">{currentPage}</span> dari <span className="font-bold text-primary-navy">{totalPages}</span>
                      </p>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 border transition-all ${
                            currentPage === 1
                              ...
                              : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50 hover:border-primary-blue shadow-xs cursor-pointer'
                          }`}
                        >
                ...
                <div className="bg-white rounded-3xl p-12 text-center border border-slate-200/80 shadow-sm">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 bg-primary-navy hover:bg-primary-blue text-white text-xs font-bold rounded-xl transition-all shadow-md cursor-pointer"
                  >
